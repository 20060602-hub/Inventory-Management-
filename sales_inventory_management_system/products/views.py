...
from sales_inventory_management_system.utils import getDropDown, dictfetchall
import datetime
...
def orderlisting(request):
    ...
    if (request.session.get('user_level_id', None) == 1):
        ...
        SQL = "SELECT * FROM `order`, `order_status` WHERE order_status = os_id AND user_id = " + customerID
    cursor.execute(SQL)
...
def payment(request):
    ...
    if (request.method == "POST"):
        # Update customer information in order table #
        cursor.execute("""
                    UPDATE `order`
                    SET order_customer_name= %s, 
                    order_customer_email= %s,
                    order_customer_contact= %s, 
                    order_customer_address= %s
                    WHERE order_id = %s
                """, (
            request.POST['customer_name'],
            request.POST['customer_email'],
            request.POST['customer_contact'],
            request.POST['customer_address'],
            orderID
        ))
            
        # Update stock in products table #
        cursor.execute("SELECT * FROM order_item WHERE oi_order_id = " + str(orderID))
        orderTotal = dictfetchall(cursor)
        for orderItem in orderTotal:
            cursor.execute("UPDATE products_product SET product_stock = (product_stock - "+orderItem['oi_cart_quantity']+") WHERE product_id = " + str(orderItem['oi_product_id']))
            print(orderItem['oi_product_id']+" ==== "+orderItem['oi_cart_quantity'])
...
def order_items(request, orderID):
    ...
    productlist = dictfetchall(cursor)
     ...
     ### Get the Cart Details Listing  ####
    cursor.execute("SELECT *  FROM `order`,`order_status` WHERE order_status = os_id  AND order_id = "+ str(orderID))
    ...
    if (request.method == "POST"):
        try:
            if(request.session.get('order_id', None) == "0" or request.session.get('order_id', False) == False):
                ...
                cursor.execute("""
                INSERT INTO `order`
                SET order_date=%s, order_status=%s, order_total=%s
                """, (
                    orderDate,
                    1,
                    0))
